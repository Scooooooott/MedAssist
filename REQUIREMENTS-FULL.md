...
系统由 **10 个服务** 构成。M0 阶段定义全部服务的模块骨架与契约;M1 阶段仅实际实现其中 5 个(标记为 ★)。
...
另有离线工具 `eval-harness`(Python),不属于运行时服务。
...
|---|---|---|---|
| **M0** | 工程地基与契约 | 10 | 规范完备的工程骨架,契约可生成双端代码,CI 绿灯 |
...
**阶段验收**:CI 全绿;`just bootstrap` 一条命令可拉起本地基础设施;契约变更可自动生成 Java 与 Python 双端代码;ArchUnit 能拦截违反模块依赖方向的代码。
...
**交付物**:parent POM、9 个 Java 模块骨架、`justfile`、端口分配表
...
2. 缓存配置:Maven 仓库缓存、uv 缓存、Docker layer 缓存。
...
1. 技术选型:本阶段使用轻量方案(Thymeleaf 或简单 React SPA)。**完整前端属于 M6.5**,不要在此过度投入。
2. 功能:
   - 提问输入框与流式回答展示
   - 引用列表展示,每条引用可展开查看原始 chunk 全文与来源元数据
   - **引用高亮**:在展开的 chunk 中高亮 `quotedSpan` 对应位置
   - 展示本次检索的元数据过滤条件(便于调试)
   - 拒答时展示明确提示与原因
3. 展示每次问答的耗时分解(嵌入 / 检索 / 生成),便于观察延迟预算。
4. 无需登录(认证属于 M4)。
...
**目标**:把 M1.11 的最小界面升级为能承载全部展示点的完整应用。

**详细需求**

1. **技术选型**:React/Next.js 或等效方案。选型考虑:静态导出可简化部署;SSR 对本场景收益有限。
2. **核心功能**:
   - 流式回答渲染,首 token 后立即展示
...
6. **工程要求**:
   ...
   - 空状态、加载态、错误态的完整处理
   - 全部文案为英文(见阶段语言策略)
7. **设计一致性**:建立最小设计系统(色板、字体、间距、组件),避免拼凑感。界面是这个项目最先被看到的部分。
