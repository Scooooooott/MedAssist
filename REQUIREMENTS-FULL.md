...
| M4 | 治理与审计 | 13 | 三角色 RBAC、策略编译器、审计链、上下文隔离、看板 |
...
M1.6/M2.6/M4.7/M4.10 提供 M4.13 的四类人工评审队列来源
```
...
| M4 | 13 | 治理、合规与审计 | 单一真相源的治理体系 + 上下文隔离 + 看板 |
...
| F7-2 | 新增 `ops-console` Django-based 内部运维控制台,范围限定为四类人工评审队列 | 反馈评审、隔离表、版本元数据确认、评估集候选提升是典型内部表驱动工作流;Django 只用于 internal-only 控制台,不替代 Java 服务或 Python 推理侧车 | §0.1、§0.3.1、§0.3.2、§0.3.4、§0.3.5、§0.4.2、M4.2、M4.3、M4.4、M4.11、M4.13、M6.2、ADR-008、ADR-009、ADR-010、附录 A/B/D/E |
