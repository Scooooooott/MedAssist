...
> 文档版本:v7.1(完整版 · 已应用可观测性、检索质量与交付面增补审阅)
...
>
> **v7.1 增补摘要**:在不重复创建既有能力的前提下,补强全链路追踪、Agent 检索轨迹、显式降级契约、重排质量指标与前端证据展示;新增 M5.12 流式生成会话与断线续传。轨迹与可观测存储不复制受控文本,公开前端不展示真实脱敏前原文。详见附录 F.8。
...
系统采用 **Java 为主、Python 为辅** 的技术路线:Java(Spring 生态)承担全部业务编排、治理、审计与 AI 编排;Python 仅承担三类无法用 Java 合格实现的模型推理能力(文档解析、PHI 检测、嵌入与重排),以无状态侧车服务形式存在。`ops-console` 是这条规则的窄例外:它使用 Django 只提供内部人工评审与运维界面,不承载线上 RAG 请求路径、业务编排或模型推理。
...
系统由 **10 个后端/运行时服务**、一个一等前端应用与一个离线内部运维控制台构成。M0 阶段定义全部服务的模块骨架、契约与前端工程脚手架;M1 阶段仅实际实现其中 5 个后端服务(标记为 ★)以及 `frontend/` 的基线问答应用。`ops-console` 在 M4 才实现,不属于 M1 范围。
...
另有前端应用 `frontend`(React + TypeScript)、内部运维控制台 `ops-console`(Python/Django,离线组件)与离线工具 `eval-harness`(Python),三者不计入上述 10 个运行时服务。
...
`ops-console` 不违反此原则:它不是请求转发服务,也不进入线上查询路径;它承载内部人工评审队列、状态流转入口与运维查看工作流,并通过 Java API 发起受审计的状态变更。
...
| 内部运维控制台 | Django | 仅 `ops-console/`,用于内部运维与人工评审控制台;版本固定于 `ops-console/pyproject.toml` |
...
├── ops-console/                     # Django 内部运维与评审控制台(M4)
├── tools/
│   └── eval-harness/
├── deploy/
...
以下为 M1 之后逐步收敛的目标,M1 阶段仅需记录实测值,不作为验收门槛。除特别标注外,单阶段目标均以 **P95** 为验收口径;P99 必须记录用于暴露尾延迟,但在首次基线建立前不作为阻塞门禁:
...
| 引用校验 | < 200 ms |
| 流恢复开始回放(M5.12) | P95 < 1 s(在会话 TTL 内,从客户端最后确认事件之后继续) |
| **端到端 P95** | **< 5 s**(触发一次重检索 < 9 s) |

正常路径与每一种降级路径必须分别统计延迟分位,禁止把跳过重排、跳过生成等功能缩水造成的低耗时计入正常路径并宣称性能提升。并行阶段的 span 会发生重叠,端到端预算应与关键路径/wall-clock 对齐,不得用所有子 span 耗时简单求和。
...
`ops-console` 仅在本地治理/内部评审场景启动,不属于线上查询路径,也不计入 M6.2 线上内存预算。若纳入本地 `governance` profile,需单独记录其常驻内存实测值。

`model-svc` 的预算必须同时覆盖实际启用的嵌入模型、线上 reranker、tokenizer 与推理运行时开销;不得只测嵌入模型后把 reranker 视为零成本。M5.12 引入的 Redis Stream 缓冲也需按最大活跃会话数、单会话事件上限与 TTL 计入容量测算。
...
- **S7** 追踪、轨迹事件、审计与流恢复缓冲不得复制原始查询、完整候选文本或其他受控文本。轨迹仅保存 ID、哈希、来源字符区间、分数、排名与决策元数据,需要查看内容时由授权服务从主存储重建。流恢复缓冲只允许保存已经通过客户端输出安全边界的事件。
...
| M4 | 治理与审计 | 13 | 三角色 RBAC、策略编译器、审计链、上下文隔离、看板、内部运维控制台 |
| M5 | 微服务基础设施 | 12 | 网关、事件流、韧性、全链路追踪、并发模型、流式会话恢复 |
...
3. **`retrieval` 服务集成**:
   ...
   - reranker 的候选输入必须是 chunk 的原始 `text`,不得拼接 M2.3 的上下文前缀;模型生成的前缀只服务于向量嵌入
4. **候选数与延迟的权衡**:需测量 N ∈ {20, 50, 100} 三档的效果与延迟,在报告中给出推荐值及理由。
5. **超时降级**:重排超时时返回未重排的融合结果并记录降级事件,**不得**因重排失败导致整个检索失败。
6. **资源约束**:线上档模型常驻内存 < 500 MB。测量必须包含模型权重、tokenizer 与推理运行时的增量常驻内存,并把启用 reranker 后的 `model-svc` 整体 RSS 回填至 M6.2。
7. **质量与尾延迟指标**:除 `context_precision` 外,报告 `precision@8`、`nDCG@8` 与对应的无重排基线;线上档记录 P50/P95/P99。P95 继续作为门禁,P99 在有稳定基线前作为观察项。
8. **自适应跳过仅作为后续实验**:可在完整基线完成后,用明确且可复现的高置信规则实验性跳过 rerank;必须与始终重排对照并证明质量无显著退化。该优化默认关闭,不属于 M2 完成前置条件。

**交付物**:model-svc 的 Rerank 实现、retrieval 集成、双档对照实验报告

**验收标准**
- 线上档对 50 候选重排 P95 < 300 ms(本机 CPU 实测)
- 开启重排后 dev split 的 `context_precision`、`precision@8`、`nDCG@8` 相对 M2.1 的变化均已量化
- 报告包含 P50/P95/P99 与包含 reranker 的增量 RSS;P99 异常长尾有解释
- 离线档与线上档的效果差距已量化并记录
- 人为使 `model-svc` 重排接口超时,检索仍能返回结果且降级事件被记录
- 专门测试证明 reranker 接收原始 `text`,上下文前缀不进入候选输入
...
4. **🔴 存储设计与消费者矩阵(必须严格,任一处混淆都会造成难以定位的故障)**:
   上下文前缀独立存储于 `chunk` 表的新字段,**不覆盖原始 `text`**。六条使用路径的取值必须明确区分:

   | 路径 | 使用哪份文本 | 混淆后的后果 |
   |---|---|---|
   | 向量嵌入 | `context + text` | —— 这是本任务的唯一目的 |
   | **词法索引(M2.1)** | **原始 `text`** | 生成词进入倒排索引,检索命中模型编造的内容 |
   | **重排输入(M2.2)** | **原始 `text`** | reranker 会把模型生成前缀误当作证据并改变排名 |
   | **最终上下文组装** | **原始 `text`** | 前缀被带入生成上下文,污染回答证据 |
   ...
   - 需有专门测试:构造一个上下文前缀中含特征词的 chunk,验证该词无法通过词法检索命中,且不会出现在 reranker 输入、最终上下文、LLM 输入、引用校验与引用展示中
...
- 三种模式均可切换并产出评估数据
- **六处边界均正确**:上下文前缀只进入向量嵌入,不出现在词法索引、重排输入、最终上下文、LLM 输入、引用校验与引用展示中(需专门测试断言)
...
4. **检查点**:每个节点执行后持久化状态快照。用途有三:
   ...
   - **运行时状态与持久化投影必须分离**:运行时 `retrievedChunks[]` 可持有本次生成所需的已脱敏/已准入文本;检查点持久化时只保存 chunk/document version ID、来源字符区间、哈希、分数、排名和选择决策,不得再次复制完整候选文本(约束 S7)
   - 从检查点恢复或评审时,由授权服务在当前角色/PDP 约束下从不可变主存储重新装载文本;恢复不得绕过已撤回文档、内容领域过滤或角色可见性
...
8. **检索轨迹完整性**:同一次请求必须能从 checkpoint 还原“各通道候选 → 融合 → 重排 → 最终上下文”的排名变化,同时记录模型/索引版本、过滤条件和降级状态。该轨迹是 M3.11 评估与 M4.10 反馈诊断的权威来源。
...
- 检查点不复制完整候选文本,但可在授权与版本约束下重建当时的候选集合和排名变化
- trace_id 可从评估失败样本定位到对应检查点、审计与追踪记录
...
10. **失败样本可诊断性**:轨迹报告的每条失败样本必须包含 `trace_id`、发生问题的阶段、候选在各阶段的排名变化和当时的降级码。报告引用候选 ID 与来源字符区间,不得复制完整候选文本。
...
- 轨迹报告包含全部六项指标
- 每条失败样本可通过 trace_id 定位到阶段级排名变化与降级原因
...
1. **清单文件** `governance/policy-manifest.yaml`,包含两大部分:
   ...
   **第三部分:角色授权**
   ...
   - 角色 → 可用的 Agent 工具集合
   - 角色 → 可访问的 `ops-console` 队列与可触发动作集合(驱动 Django 专属数据库角色与 Java API 授权)
   - 角色 → 可读取的轨迹投影、可执行的流会话动作(`create` / `resume` / `cancel`)及作用域;用户只能操作本人或当前匿名会话拥有的生成会话
...
**目标**:把一份清单编译为七类可执行产物,实现"一处定义、多处生效"。
...
2. **七类输出产物**:
   ...
   | Django 专属数据库角色的 RLS 策略与 GRANT | SQL 脚本 | Postgres / `ops-console`(M4.13) |
   | 应用级资源/动作授权表 | 结构化配置 | 轨迹查询(M4.10)、网关/BFF 与流会话(M5.1/M5.12) |
...
- Django 专属数据库角色的可见范围符合清单定义,且不得拥有绕过 RLS 的权限
- 黄金文件测试通过
...
5. **豁免机制**:允许在清单中显式声明豁免列表(如 Spring Batch 的 `BATCH_*` 系统表与 Django 的 `django_*` 系统表),但豁免必须逐项写明理由,且豁免列表本身纳入评审。
   - `BATCH_*` 豁免理由:Spring Batch 自有元数据表,不承载业务 PHI 或治理资源。
   - `django_*` 豁免理由:Django-based ops console 自有会话、权限与迁移记录表,只支撑 `ops-console` 内部控制台运行,不属于业务数据 schema;业务表仍必须由清单覆盖。
...
3. **上下文关联**:每条反馈关联 `trace_id`,并保证能通过 M3.2 的检查点重建当时的完整召回上下文与 Agent 轨迹。反馈脱离上下文则无法分析。
   ...
   - 提供一次调用即可完成的受控轨迹查询:输入 `trace_id`,返回经裁剪的节点时间线、候选 ID/来源区间、通道分数与排名变化、最终选择、模型版本、过滤条件和降级码
   - 轨迹接口不得直接暴露 tracing backend、checkpoint 表或完整 span 属性;文本内容按当前访问者身份从主存储重建,并再次执行撤回状态与内容领域授权检查
4. **评审队列**(ADMIN 界面):
...
- 轨迹重建通过受控服务接口完成,调用方不能绕过授权直读 checkpoint 或追踪存储
...
6. **全组合参数化测试**:角色 × 资源类型 × 动作的完整笛卡尔积,逐项断言允许或拒绝。
   - 资源类型:政策文档、临床记录、结构化个案、结构化聚合、审计记录、治理看板、评估集、轨迹投影、生成会话、`ops-console` 队列与动作
   - 动作:检索、查询、读取、导出、创建、恢复、取消(不适用的组合应明确拒绝,仍纳入矩阵)
...
1. **需要传播的上下文清单**(缺一不可):
   ...
   M4 阶段先交付统一的载荷注册接口与测试替身;M5.4 接入 OTel 后将真实 trace/span 上下文注册到同一机制。不得为 OTel 另建第二套传播框架。
...
## M4.13 内部运维控制台(Django)

**目标**:为内部运维与人工评审提供一个严格受治理约束的离线控制台。

**背景与动机**:Django 只用于内部运维与评审控制台,不替换任何 Java 服务,也不替换 Python 推理侧车。项目已经积累四类需要人工看表、筛选、改状态的场景:M4.10 的反馈评审队列、M1.6 / M4.7 的隔离表、M2.6 的文档版本元数据人工确认队列、M1.12 评估集候选的人工提升流程。这些是典型的表驱动 CRUD + 状态流转场景,Django-based ops console 能以较少代码生成可用内部界面;这是用对工具,不是扩大主架构技术栈。

**详细需求**

1. **范围限定**:
   - 反馈评审队列(M4.10)。
   - 隔离表条目(M1.6 解析失败、脱敏失败;M4.7 质量断言失败)。
   - 文档版本元数据人工确认队列(M2.6)。
   - 评估集候选的人工提升流程(M1.12 / M4.10)。
   - 不得扩展为通用后台、业务管理系统或线上用户界面。
2. **🔴 schema 归属**:
   - Flyway 是唯一业务 schema 所有者(M1.8)。
   - Django 使用 `inspectdb` 生成 `managed = False` 模型。
   - Django 永不执行业务表迁移。
   - CI 必须检查业务表不存在 Django 迁移文件。
3. **🔴 不得绕过治理**:
   - Django 使用专属数据库角色连接。
   - 该角色的 RLS 策略与 GRANT 由 M4.3 策略编译器生成。
   - 禁止使用超级用户、表 owner 或任何绕过 RLS 的连接。
   - 内部工具纳入统一策略体系,证明"权限在数据库层强制"不是只对外部路径成立。
4. **🔴 对业务表只读**:
   - Django 可读取业务表用于列表、筛选、详情页与只读检查。
   - 状态变更(反馈提升、隔离条目重处理、版本元数据确认、评估候选提升)必须调用 Java API。
   - Django 不得直接写业务表或审计表;否则会形成第二写入者并破坏审计链完整性。
5. **🔴 离线组件**:
   - `ops-console` 不作为线上组件进入 M6.2 的上线集合;可在 M6.2 表中标明不上线。
   - 不公网暴露,仅本地或受控内网运行。
   - 不计入线上内存预算。
6. **审计要求**:
   - 所有操作必须产生审计事件。
   - 审计事件经 Java API 落库,不得由 Django 直接写审计表。
   - 审计 payload 不含 PHI 原文(约束 S1)。
7. **Django 自有表**:
   - `django_*` 系统表加入 M4.4 漂移检查豁免清单。
   - 豁免理由必须说明这些表只支撑 Django-based ops console 自身,不承载业务数据。
8. **界面与权限**:
   - 控制台界面文案为英文(约束 G9)。
   - 可见数据范围由数据库 RLS 与 Java API 授权共同约束,不得在 Django 中维护独立权限定义(约束 P1)。
   - Django `superuser` / `staff` 只用于框架后台登录能力,不得被视为项目 `ADMIN` 角色;项目权限必须映射到 `policy-manifest.yaml`。
   - 列表页需显示当前队列状态、严重度、失败阶段、更新时间与可执行动作。
   - `ops-console` 不替代 M4.9 的 React 治理/质量/成本看板;M4.9 面向 demo 展示与角色可见性,本任务面向内部人工处理。
9. **状态变更 API 粒度**:
   - 每类队列的状态变更必须有明确 Java API 契约。
   - 若发现现有 Java API 粒度无法表达所需操作,需停下提出需求变更,不得绕过 API 直接写库。

**交付物**:`ops-console/` Django 项目、`inspectdb` 生成的 `managed = False` 模型、只读内部控制台页面、Java API 调用适配、Django 专属数据库角色策略、审计集成、CI 迁移检查、ADR-008/009/010

**验收标准**
- `ops-console` 只能通过专属数据库角色连接,且该角色无绕过 RLS 权限
- 业务表模型均为 `managed = False`
- CI 能拦截任何针对业务表的 Django 迁移文件
- Django 对业务表的数据库连接权限为只读;状态变更全部经 Java API 完成
- 人为尝试在 Django 中直接更新业务表被数据库拒绝
- 四类队列均可在控制台中查看、筛选并触发对应 Java API 操作
- 每次操作均产生审计事件,且审计事件不由 Django 直接写表
- 界面文案全部为英文
- `django_*` 系统表已在 M4.4 豁免清单中声明理由

**依赖**:M4.3, M4.4, M4.10, M1.6, M1.12, M2.6, M4.7
...
9. **流会话与诊断 BFF 边界**:
   - 为 M5.12 的“认证 POST 创建生成会话 + 认证 GET 订阅/恢复事件流”提供路由、限流、同源 Cookie 或令牌校验能力
   - 不得假设浏览器原生 `EventSource` 可以携带任意 `Authorization` 头;认证形态由 D34 决策并写入 ADR
   - 轨迹与追踪展示只能读取服务端裁剪后的投影,禁止把 tracing backend、checkpoint 表或内部管理端点直接暴露给浏览器
10. 服务端口 8080。
...
- 流恢复 GET 请求会重新校验会话所有权,不能通过猜测生成 ID 读取其他会话事件
...
2. **Topic 设计**:
   ...
   | `trajectory-events`(可选) | `agent` / `retrieval` | 轨迹投影消费者 | 按 trace_id |

   `trajectory-events` 不是 M3.2 checkpoint 的替代品。只有出现明确的跨服务实时消费需求并经 ADR 确认后才启用;事件只携带候选 ID、来源区间、分数、排名、模型版本与决策元数据,不得携带查询或候选完整文本。未启用时,M4.10 直接从 checkpoint 的受控投影重建轨迹。
...
8. **保留策略**:各 topic 的保留期配置化且按数据用途分别定义。`audit-events` 与 M4.6 审计策略一致;`feedback-events` 与 M4.10 评审窗口一致;可选 `trajectory-events` 与 D30/D36 的 checkpoint/反馈窗口一致且不得更长;禁止为了配置方便把全部 topic 统一成最长保留期。
...
1. **🔴 逐组件的失败行为定义(核心表格)**:
   ...
   | `model-svc` 嵌入 | **默认失败** | lexical-only 只有在 D35 的离线质量门槛通过、显式启用且客户端能识别降级结果后才允许;禁止静默切换 |
   ...
   | LLM 供应商 | **切换备用供应商**(M5.6);全部不可用时默认明确失败 | “只返回检索证据”的终止响应仅在 D35 决策通过后启用,且必须经过客户端输出安全边界 |
...
7. **结构化降级契约**:所有用户可见降级统一返回 `code`、`affected_stage`、`fallback_mode` 与安全的 `reason`。同一降级码必须同步写入响应、trace、checkpoint/轨迹与审计事件,不得只写日志或静默改变执行路径。
8. **受控可选降级**:
   - lexical-only 模式必须使用独立结果码 `LEXICAL_ONLY_DEGRADED`,通过 dev split 质量门槛且保持引用与过滤约束;默认关闭
   - LLM 全不可用时的“检索式终止响应”只能返回已脱敏、已授权、可精确引用的来源片段,不得伪装成生成答案;默认仍为明确失败
   - 两项能力均需 ADR、独立开关、故障注入与正常/降级指标分桶,未满足任一条件时不得启用
...
- 全部降级行为均产生审计事件
- 用户可见降级在响应、trace、轨迹与审计中的 code 一致,不存在静默降级
...
3. **Span 划分**:每个关键阶段一个 span——网关路由、鉴权决策、查询脱敏、查询嵌入、向量检索、词法检索、重排、工具调用、LLM 生成、引用校验、出站闸门。span 命名遵循统一约定。
   - 并行向量/词法检索必须表现为同一父 span 下的 sibling spans,不得串联成虚假的顺序关系
   - Python 推理至少区分排队等待与实际推理两个 span/事件,使 M5.11 能判断瓶颈来自背压还是模型计算
4. **🔴 Span 属性不得包含 PHI(约束 S1)**:
   ...
   - 当前项目为单租户,不得为了“未来扩展”记录原始 `tenant_id`;如需区分 demo 访问范围,使用低基数角色与不可逆匿名会话标识
...
7. **采样策略**:配置化。demo 场景可全采样,但配置项必须存在并文档化。若要求错误/慢请求全采样,必须在 Collector 使用 tail sampling 并验证其资源开销;未实施 tail sampling 时不得宣称具备该能力。
8. **开销与基线**:记录关闭/开启 tracing 的吞吐、P95/P99 与 CPU/内存差异;在 M5.10/M5.11 调优前归档各关键阶段的基线,供后续瓶颈判断。
9. **验证**:一次完整问答请求,能在追踪界面中看到从网关到 Python 服务再返回的完整链路。端到端耗时与关键路径/wall-clock 基本吻合;并行 sibling spans 的重叠必须被正确解释,不得要求所有子 span 简单求和等于总耗时。
...
- 并行检索以 sibling spans 展示,Python 推理可区分排队与执行
- 关键路径/wall-clock 与端到端耗时差异在合理范围内,并行重叠解释正确
- tracing 开销与采样配置已有实测报告,span 属性白名单使用固定安全夹具回归
...
2. **指标分类**:
   ...
   - **降级**:按 `degradation.code`、阶段与 fallback mode 分桶的请求数、错误率和延迟;正常路径与降级路径分开
   - **追踪关联**:关键延迟直方图与错误计数支持 exemplar 跳转到对应 trace,不得把 trace_id 作为普通高基数指标标签
...
- 人为触发脱敏泄漏,最高级告警正确触发
- SLO 错误预算可查询
- 可从关键延迟/错误指标的 exemplar 跳转到对应 trace
- 告警规则以配置形式提交仓库
- 与 M4.9 无重复实现的指标
...
1. **故障场景矩阵**,逐项验证 M5.3 的表格:
   ...
   | 数据库连接池耗尽 | 舱壁快速拒绝,不让全部请求排队至全局超时 | 高 |
   | 向量或词法单分支超时 | 按 M5.3 的已批准语义失败或显式降级,不得静默缺失 | 中 |
...
8. **降级可见性验证**:对允许降级的每个场景,断言响应、trace、checkpoint/轨迹和审计事件中的降级 code 一致;正常与降级路径的延迟分位分别归档。
9. **可选路径门禁**:若启用 lexical-only 或检索式终止响应,必须增加质量阈值、内容授权、精确引用和客户端输出安全边界测试;未启用时则验证系统维持默认明确失败。
...
- 连接池耗尽与单检索分支超时均已覆盖,不存在静默降级
- 每个降级场景在响应、trace、轨迹与审计中的 code 一致
...
2. **🔴 瓶颈迁移分析(本任务的核心产出)**:
   ...
   - 分析必须以 M5.4 的阶段 span 基线为证据,区分数据库等待、下游排队、网络等待与实际计算;不得只用端到端平均值猜测瓶颈
...
7. **实测与调优**:
   ...
   - 使用 M5.4 的排队/执行 span 证明调优改善来自等待缩短还是推理吞吐提升,并记录 P99 长尾
...
**依赖**:M0.5, M1.2, M1.4, M5.3, M5.4

---

## M5.12 流式生成会话与断线续传

**目标**:把 M1.10 的单连接 POST/SSE 基线升级为有身份绑定、可恢复、可取消且资源有界的生成会话,使短暂断网或页面刷新不会丢失已批准的输出事件。

**背景与动机**:M1 的浏览器 `fetch` POST 流在连接中断时会取消后端订阅,也没有稳定事件 ID。原生 `EventSource` 只能发 GET 且不能自由设置 `Authorization` 头,因此仅增加 `Last-Event-ID` 不能完成安全续传。恢复能力必须先定义会话资源、认证协议与客户端输出安全边界。

**详细需求**

1. **两阶段协议(契约先行)**:
   - 认证 `POST /api/generations` 创建生成会话,请求包含查询、过滤条件与幂等键;返回不可预测的 `generation_id`、状态、过期时间和事件流地址
   - 认证 `GET /api/generations/{id}/events` 建立或恢复 SSE;客户端通过 `Last-Event-ID` 或等价显式游标请求从最后确认事件之后继续
   - `POST` 重试使用同一幂等键时不得创建第二次 LLM 调用;契约与兼容策略先在 `contracts/` 和 API 文档中定义
2. **浏览器认证形态**:二选一并写入 ADR(D34):同源 BFF + HttpOnly/SameSite Cookie 后使用原生 `EventSource`,或继续使用支持请求头的 fetch-SSE。不得把访问令牌放入 URL、事件数据或可被前端日志记录的位置。
3. **Redis Stream 事件日志**:
   - 使用 Redis Stream 的原生事件 ID、范围读取和裁剪能力,不使用普通 List 模拟游标
   - 每个事件包含 `event_id`、`generation_id`、`type`、安全 payload、创建时间与 schema version
   - 至少支持 `accepted`、`delta`、`citation`、`degradation`、`final`、`error`、`cancelled` 事件;`final`/`error`/`cancelled` 为终态且可重放
4. **会话状态机**:`CREATED → RUNNING → COMPLETED / FAILED / CANCELLED / EXPIRED`。状态转换须幂等且原子;恢复连接不能把终态会话重新启动。
5. **所有权与授权**:
   - 会话绑定 M4.1 用户身份或 M6.4 匿名会话标识、角色和创建时策略版本
   - 每次订阅、恢复、查询状态或取消都重新校验所有权与策略,禁止仅凭 `generation_id` 访问
   - 角色变化或权限收紧时不得复用旧角色的事件流;按策略拒绝或只返回允许公开的终态摘要
6. **断开与取消语义**:
   - 网络断开不立即取消生成,但只允许在配置的后台继续窗口内运行
   - 用户显式取消必须尽快终止下游生成、写入 `cancelled` 终态并释放资源
   - 定义同一会话多客户端订阅和重复恢复的行为;客户端按事件 ID 去重
7. **资源与成本上限**:配置单会话最大时长、最大 token/事件数、最大缓冲字节数、每用户/会话最大活跃生成数、全局活跃上限与 TTL。超过任一上限时进入明确终态并产生结构化降级/错误事件,禁止断线后无限生成。
8. **客户端输出安全边界**:
   - M3.9 保护的是发往外部 LLM 的内容,**不等于**客户端输出已经安全;必须单独定义“可提交到流缓存”的输出审批边界(D37)
   - Redis Stream 只写入已通过该边界的客户端事件,不得写入原始用户输入、外部 LLM 请求 payload、未批准草稿或完整候选文本
   - 若无法对逐 token 输出可靠执行安全检查,必须改用经过检查的有界文本块或只在最终响应通过后提交,不得以体验为由先发后查
9. **可观测与保留**:
   - 指标包括活跃会话数、恢复次数、恢复延迟、缓冲字节数、过期/取消数、后台继续时长与重复事件数
   - trace 关联创建、生成、事件写入与恢复读取,但 span 属性不含事件正文
   - 流缓冲按分钟级 TTL 管理,不是审计记录;终态与审计只保留必要元数据。具体值写入保留策略并纳入容量测算
10. **前端集成契约**:本任务交付可自动恢复的客户端状态机与底层 API;M6.5 负责最终交互打磨。页面刷新、短暂离线和重复终态事件不得造成重复文本、重复引用或状态倒退。

**交付物**:生成会话 API 与契约、Redis Stream 存储、状态机、认证恢复、取消与清理、资源上限、客户端恢复库、ADR、集成测试

**验收标准**
- 创建接口的幂等重试只触发一次生成
- 在至少产生 3 个事件后断开并携带最后事件 ID 恢复,客户端无丢失、无重复地收到余下事件与终态
- 页面刷新后可恢复本人会话,但无法读取另一用户/匿名会话的事件
- 网络断开仅在配置窗口内后台继续;显式取消会终止下游并释放资源
- Redis Stream 与日志均不含原始查询、未批准草稿或完整候选文本
- 终态事件可重放,过期会话返回明确 RFC 9457 错误而非重新生成
- 最大时长、token、事件、字节数和活跃会话上限均有自动化测试
- 恢复开始回放 P95 满足 §0.4.1,并记录正常连接与恢复连接的独立指标

**依赖**:M1.10, M2.7, M3.1, M3.9, M4.1, M5.1, M5.3
...
4. **网关日志同样是泄漏路径**。默认关闭请求体日志,调试时开启也必须走脱敏。span、网关日志、checkpoint/轨迹投影与 M5.12 流缓冲是 PHI 最容易漏出的四类位置,均需白名单或客户端输出审批边界。
...
17. **断线不等于取消**。连接生命周期、生成生命周期与流缓冲生命周期是三件事。M5.12 必须分别建模并设置上限,否则短暂断网会浪费已完成工作,而无限后台继续又会变成成本与滥用风险。
...
1. **在线 / 离线划分**:
   ...
   | `parser-svc` | ❌ | 仅摄取阶段使用 |
   | `eval-harness` | ❌ | 离线工具 |
   | `ops-console`(Django) | ❌ | 内部运维工具,仅本地运行 |
   | Postgres / Redis | ✅ | |
   | Keycloak | ✅ | 认证 |
   | Redpanda | ⚠️ | 见下方决策点 |
...
3. **🔴 内存预算必须实测,不得估算——但先看这份参考基线**:
   ...
   `model-svc` 的实测值必须明确列出启用的嵌入模型、线上 reranker、tokenizer 与推理运行时;若参考值未包含实际 reranker,必须更正而非沿用 1500 MB。Redis 的实测值需同时覆盖普通缓存与 M5.12 在最大允许活跃会话下的 Stream 缓冲。
...
2. **核心功能深化**:
   ...
   - **证据并列视图**:答案与来源文档可并列阅读;点击引用按 `document_version_id + source_char_start/source_char_end` 定位到精确字符区间,不得用模糊全文搜索代替
   ...
   - **断线恢复**:接入 M5.12 的生成会话状态机,页面刷新或短暂断网后从最后确认事件继续;按事件 ID 去重,终态重放不得重复追加文本或引用
3. **Agent 调试/透明面板**(可折叠,默认收起):
   - 本次使用的检索模式、过滤条件、召回数量
   - Agent 执行路径(路由结果、调用的工具、是否重试)
   - 各阶段耗时分解
   - 检索管线视图:向量/词法通道排名、RRF 融合排名、重排前后名次变化与最终入选状态
   - 经服务端裁剪的轨迹时间线:节点、工具、重试、降级码和关键耗时;不得直接展示完整 span 属性、checkpoint 原始结构或内部追踪后端
...
7. **设计一致性收尾**:在 M0.11 的最小设计系统基础上统一色板、字体、间距、状态样式与页面布局,避免拼凑感。界面是这个项目最先被看到的部分。
   - 采用密集、工具型、低装饰的界面,优先支持扫描、对比与重复操作;动效尊重 `prefers-reduced-motion`
   - 沿用既有设计系统与样式实现,不强制迁移到 Tailwind 或其他 CSS 框架;若迁移必须另写 ADR 并证明收益大于 bundle 与重构成本
8. **脱敏对照的安全边界**:
   - 公共前端只允许使用明确标记的合成 fixture 演示“脱敏前/后”差异,不得展示任意真实输入或受控源文档的脱敏前原文
   - 若未来需要审核真实脱敏结果,应单独扩展 M4.13 的内部控制台、权限矩阵与审计,不得复用公共 demo 页面
9. **数据访问边界**:证据、轨迹、排名和追踪摘要均通过受控 BFF/API 获取;前端不得直连数据库、Redis、checkpoint 存储、Tempo/Jaeger 或内部管理 API。
...
- 引用可在并列来源视图中定位到精确字符区间
- 检索管线可展示通道、融合、重排与最终选择的排名变化
- 断线或刷新后按事件 ID 恢复,不丢失且不重复渲染内容
- 公共脱敏对照仅使用合成 fixture,浏览器无法访问真实脱敏前原文或完整内部 trace
...
**依赖**:M1.11, M6.4, M4.9, M4.10, M5.3, M5.4, M5.12
...
M4.3/M4.4/M4.10 ──► M4.13(内部运维控制台)
M1.6/M1.12/M2.6/M4.7/M4.10 提供 M4.13 的四类人工评审队列来源
```
...
**M4.13(内部运维控制台)** 依赖 M4.3/M4.4 的治理边界和 M4.10 的反馈队列,可在 M4.3 产出 Django 专属数据库角色策略后并行推进,但不进入线上查询路径。
...
M1.10/M2.7/M3.1/M3.9/M4.1/M5.1/M5.3 ──► M5.12 ──► M6.5
...
```
M5.3 + M4.12 + M5.4 ──► M5.10 虚拟线程 ──► (回归 M4.12 压力测试)
M0.5 + M1.2/M1.4 + M5.3 + M5.4 ──► M5.11 Python 并发调优 ──► (回填 M6.2 内存预算)
```
...
- [ ] ArchUnit 可拦截跨服务直接依赖
- [ ] 7 篇 ADR 完成,每篇含被否决方案
- [ ] `frontend` 脚手架可完成 lint、typecheck、test、build 与体积预算检查
...
- [ ] React 前端 SSE 增量 markdown 渲染无 DOM 结构错误
- [ ] React 前端 `quotedSpan` 高亮通过空白、换行、大小写归一化用例
- [ ] 召回列表 500 个 mock chunk 下启用虚拟滚动
- [ ] 前端拒答、空态、加载态、网络错误与 SSE 中断均有组件测试覆盖
- [ ] 前端文案全部为英文,组件测试覆盖率达到 M1.11 阈值
- [ ] 无答案题触发正确拒答
...
- [ ] 重排报告含 `precision@8`、`nDCG@8`、P99 与包含 reranker 的增量 RSS
- [ ] Contextual Retrieval 成本估算报告在实施前完成
- [ ] **上下文前缀六处边界正确**:除向量嵌入外,不入词法索引、重排输入、最终上下文、LLM 输入、引用校验与展示
- [ ] 词法索引建在原始 `text` 上
...
- [ ] 编排图可导出且与代码一致
- [ ] 检查点快照不含 PHI 原文
- [ ] 检查点不复制完整候选文本,但可按授权重建候选集合和各阶段排名变化
- [ ] 硬性终止条件生效,无无限循环可能
...
- [ ] 轨迹失败样本含 trace_id、阶段诊断、排名变化与降级码
...
- [ ] 轨迹通过受控服务接口投影,调用方无法直读 checkpoint 或追踪存储
- [ ] **代码中不存在任何反馈自动回流路径**
...
- [ ] `ops-console` 只能通过专属数据库角色连接,且该角色无绕过 RLS 权限
- [ ] `ops-console` 业务表模型均为 `managed = False`,CI 可拦截业务表 Django 迁移文件
- [ ] Django 对业务表只读,状态变更全部经 Java API 完成
- [ ] 四类内部评审队列均可查看、筛选并触发对应操作
- [ ] `ops-console` 操作均产生审计事件,且不由 Django 直接写审计表
- [ ] `django_*` 系统表已在漂移检查豁免清单中声明理由
- [ ] `ops-console` 界面文案全部为英文
...
- [ ] 舱壁生效,单一慢下游不耗尽全服务线程
- [ ] 全部降级行为产生审计事件
- [ ] 用户可见降级在响应、trace、轨迹与审计中的 code 一致,正常/降级指标分桶
- [ ] 完整链路可在追踪界面还原,含 Python 服务 span
- [ ] 异步审计处理可关联回原始请求
- [ ] **span 属性中不含查询文本或文档内容**(硬性)
- [ ] trace_id 在追踪、审计、反馈、检查点四处一致
- [ ] 并行检索以 sibling spans 展示,Python 推理可区分排队等待与执行
- [ ] tracing 开销、采样策略与阶段基线已有实测记录
- [ ] 延迟分阶段面板可定位慢请求耗时集中环节
- [ ] 三条合规告警(泄漏 canary / 审计链断裂 / 越权访问)可正确触发
- [ ] SLO 错误预算可查询
- [ ] 关键延迟或错误指标可通过 exemplar 跳转到 trace
- [ ] 切换 LLM 供应商仅需修改配置
- [ ] 硬预算阈值触发时新请求被拒绝
...
- [ ] 运行手册覆盖全部故障模式
- [ ] 创建生成会话的幂等重试只触发一次生成
- [ ] SSE 断开后可从最后事件 ID 恢复,无事件丢失或重复渲染
- [ ] 流会话恢复重新校验所有权,无法读取其他用户/匿名会话的事件
- [ ] 流缓冲不含原始查询、未批准草稿或完整候选文本
- [ ] 流会话时长/token/事件/字节数/活跃数上限与 TTL 均已验证
...
- [ ] `model-svc` 内存实测包含线上 reranker,Redis 实测包含最大允许流缓冲
...
- [ ] 部署与回滚流程文档可被他人照做
- [ ] 生产密钥不在仓库中
- [ ] **含 PHI 的查询后,日志/审计/检查点/缓存/反馈/流缓冲六处均无原文残留**(硬性)
- [ ] 出站闸门拦截提示不回显检测内容
- [ ] 免责声明首屏可见
...
- [ ] 重置脚本可恢复干净状态
- [ ] 流式渲染、引用高亮、拒答展示正常
- [ ] 角色切换后客户端已缓存的检索结果、回答、引用展开状态与透明面板数据全部失效
- [ ] 当前角色可用工具以标签形式展示,且与策略清单一致
- [ ] M4.9 三张看板按角色可见性规则集成
- [ ] M4.10 引用级反馈组件可在引用项上提交反馈
- [ ] 透明面板展示 Agent 执行路径与耗时分解
- [ ] 引用可在并列来源视图定位到精确字符区间
- [ ] 检索管线展示双通道、融合、重排与最终选择的排名变化
- [ ] 页面刷新/短暂断网后恢复流式回答且不重复文本或引用
- [ ] 公共脱敏前后对照只使用合成 fixture,不暴露真实原文或内部 trace
- [ ] 移动端布局可用
- [ ] 五类安全扫描接入 CI 且无高危项
- [ ] **Git 历史密钥扫描通过**
- [ ] SBOM 随构建自动产出
...
| **S7** | 追踪、轨迹、审计与流缓冲不复制受控文本;内容按授权从主存储重建 |
| **G1** | 契约先行:先定义再实现 |
...
| **R11** | 降级必须在响应、trace、轨迹与审计中显式且使用同一 code,禁止静默降级 |
| **R12** | 断线后后台生成必须受时长、token、事件、字节数与并发上限约束 |
...
| **E5** | 模型生成的辅助内容只服务于其声明目标路径;Contextual Retrieval 前缀仅进入向量嵌入,不得进入词法索引、重排、最终上下文、LLM 输入或引用链路 |
...
**PHI 最容易漏出的六类位置**:M3.2 的 Agent 检查点/轨迹投影、M5.1 的网关请求日志、M5.4 的追踪 span 属性、M5.12 的流缓冲、M6.4 的访客输入留存以及反馈自由文本。前四类采用白名单或客户端输出审批边界(R4/S7),其余路径依托 M3.1 的入口脱敏(E6)与 M4.10 的反馈脱敏——**入口脱敏一旦生效,这些位置的风险会同时下降一个量级**,但仍不得以此取消各存储边界的独立检查。
...
| D32 | Vite 与 Next.js 静态导出的选择 | M0.11, M1.11, M6.5 | 已倾向 Vite;实现前确认部署与路由需求未变化,结论回写 ADR-007 |
| D33 | Django 只读约束下状态变更 API 的粒度 | M4.13, M4.10, M1.6, M2.6, M4.7 | 若现有 Java API 无法表达控制台动作,需先补契约而非让 Django 直写业务表 |
| D34 | 浏览器可恢复 SSE 的认证与传输形态 | M5.1, M5.12, M6.5 | 同源 HttpOnly Cookie + EventSource vs 携带请求头的 fetch-SSE,不得把令牌放入 URL;结论写入 ADR |
| D35 | 向量失败时 lexical-only、LLM 全不可用时检索式终止响应是否启用 | M5.3, M5.9 | 默认关闭;以 dev split 质量、安全边界、引用完整性和用户可识别的结构化结果为启用门槛,结论写入 ADR |
| D36 | 轨迹是否需要 Redpanda 事件投影及其保留期 | M3.2, M4.10, M5.2 | checkpoint 为权威来源;仅有跨服务实时消费需求时启用事件,保留期不得长于 checkpoint/反馈窗口且不含完整文本 |
| D37 | 流式客户端输出的审批粒度 | M3.9, M5.12 | 逐 token、经检查的有界文本块或 final-only 三选一;以“未批准内容永不进入 Redis/浏览器”为硬约束,写入 ADR |
...
| 阶段 | 任务数 | 主题 | 交付状态 |
|---|---|---|---|
| M0 | 11 | 工程地基与契约 | 规范完备的工程骨架 |
| M1 | 14 | 基线 RAG | 可问答、有引用、有基线分数 |
| M2 | 9 | 检索工程 | 有实验支撑的优化 + CI 门禁 |
| M3 | 11 | Agent 与工具 | 路由、工具、引用闸门、出站闸门 |
| M4 | 13 | 治理、合规与审计 | 单一真相源的治理体系 + 上下文隔离 + 看板 + 内部运维控制台 |
| M5 | 12 | 微服务基础设施 | 网关、事件、韧性、追踪、并发模型、流式会话恢复 |
...
- **M1 结束**(25 项):已是一个完整可讲的 RAG 项目
- **M4 结束**(58 项):治理与合规叙事完整,项目的核心差异化已全部到位
- **M6 结束**(78 项):正式交付状态,可直接投递
...
| 节点 | 累计任务 | 全职估算 | 业余估算 |
|---|---|---|---|
| M1 结束 | 25 | 6–9 周 | 3.5–5.5 个月 |
| M4 结束 | 58 | 4–5 个月 | 10–15 个月 |
| M6 结束 | 78 | 6.5–8.5 个月 | 1.5 年以上 |
...
| A5 | Contextual Retrieval 的上下文前缀在词法索引、重排、上下文组装、LLM 输入、引用校验与展示中的归属不明 | 明确**六类消费者边界**:仅向量嵌入使用 `context + text`,其余路径一律用原始 `text` | M2.1、M2.2、M2.3、M3.7 |
...
| 编号 | 变更 | 原因 | 影响范围 |
|---|---|---|---|
| F7-1 | React + TypeScript 从 M6.5 前移到 M1.11,并新增 M0.11 前端工程规范与脚手架 | 一套前端代码从 M1 演进到 M6 比先写后弃更省,且 React 是目标岗位能力信号;M6.5 改为深化与打磨而非重写 | §0.3.4、§0.3.5、§0.6、M0.1、M0.7、M0.8、M0.11、M1.11、M6.5、ADR-007、附录 A/B/D/E |
| F7-2 | 新增 `ops-console` Django-based 内部运维控制台,范围限定为四类人工评审队列 | 反馈评审、隔离表、版本元数据确认、评估集候选提升是典型内部表驱动工作流;Django 只用于 internal-only 控制台,不替代 Java 服务或 Python 推理侧车 | §0.1、§0.3.1、§0.3.2、§0.3.4、§0.3.5、§0.4.2、M1.6、M1.12、M2.6、M4.2、M4.3、M4.4、M4.7、M4.10、M4.11、M4.13、M6.2、ADR-008、ADR-009、ADR-010、附录 A/B/D/E |

## F.8 v7.1 可观测性、检索质量与交付面增补

| 编号 | 增补审阅结论 | 处理 | 影响范围 |
|---|---|---|---|
| F8-1 | 分布式追踪已由 M5.4 覆盖,不应提前为 M1 的重复任务 | 补充并行 sibling spans、Python 排队/推理拆分、tail sampling 前置条件、开销基线与 Prometheus exemplar;M5.10/M5.11 增加 M5.4 依赖 | §0.4.1、M4.12、M5.4、M5.5、M5.10、M5.11 |
| F8-2 | Agent 轨迹应基于 M3.2 checkpoint,不应把完整候选文本复制进审计或事件总线 | checkpoint 持久化投影只存 ID、来源区间、分数、排名和决策;M4.10 提供受控重建;Redpanda 轨迹事件仅为可选投影 | S7、M3.2、M3.11、M4.10、M5.2 |
| F8-3 | 降级行为主体已在 M5.3/M5.9,且 M3.6 不是通用韧性任务 | 增加统一降级 code、跨响应/trace/轨迹/审计一致性、连接池与单分支故障;lexical-only 和检索式终止响应默认关闭并进入 D35 | M5.3、M5.9、R11、D35 |
| F8-4 | 两阶段重排已由 M2.2 覆盖 | 增加 `precision@8`、`nDCG@8`、P99、reranker 增量 RSS,并把原始文本消费者矩阵扩为六类 | M2.2、M2.3、M6.2 |
| F8-5 | 断线续传是唯一需要新增的独立任务 | 新增两阶段生成会话协议、Redis Stream、认证恢复、幂等/取消/上限与客户端输出安全边界 | M5.1、M5.12、M6.5、D34、D37 |
| F8-6 | React 前端与透明面板已存在,无需另建 Web 交付任务 | M6.5 增加精确来源定位、排名变化、受控轨迹、恢复体验;真实脱敏前原文只允许进入另行授权的内部控制台,公共页面仅用合成 fixture | M4.10、M6.5 |
